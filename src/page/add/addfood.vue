...
<script>
import { getShopcategroy, postUpdateFood, addFood } from '@/api/api.js'
export default {
  data () {
    return {
  ...
  methods: {
    async initData () {
      try {
        const result = await getShopcategroy(this.restaurant_id)
        if (result.status === 1) {
    ...
    submitcategoryForm (categoryForm) {
      this.$refs[categoryForm].validate(async (valid) => {
        if (valid) {
          ...
          try {
            const result = await postUpdateFood(params)
            if (result.status === 1) {
              this.initData()
